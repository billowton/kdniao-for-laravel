...
class Kdniao
{
    ...
    public function getOrderHandle($requestData)
    {
        if (!$this->is_not_json($requestData))
        {
            return '$requestData,必须是json类型';
        ...
        $result = $this->sendTracesPost($datas, $url);
     ...
     * @param $logisticCode,快递单号
     */
    public function orderHandle($logisticCode)
    {
        $json = "{\"LogisticCode\":'$logisticCode'}";
        return $this->getOrderHandle($json);
    }
     ...
     * @return string,
     */
    private function sendTracesPost($datas, $url = "http://api.kdniao.cc/Ebusiness/EbusinessOrderHandle.aspx")
    {
        $temps = array();
